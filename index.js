...
function addTodo() {
  ...
  let text = prompt("add your list todo : ");
  ...
  console.log(`Success added to-do ${objectTodo.text}`);
}
...
function markTodoCompleted() {
  ...
  let numberTodo = prompt("Input Todo Number : ");
  let numberTodos = Number(numberTodo);
  ...
  todos[numberTodos - 1].isCompleted = true;
...
function deleteTodo() {
  ...
  let deleteIndexNumber = prompt("Number delete todo : ");
  let deleteNumberTodo = Number(deleteIndexNumber);
  // 3. Validasi input: Pastikan nomor adalah angka, dalam rentang yang valid
  if (
    deleteIndexNumber.trim() === "" ||
    deleteNumberTodo < 1 ||
    deleteNumberTodo > todos.length ||
    isNaN(deleteNumberTodo)
  ) {
    console.log("Invalid number. Please enter a valid number from the list.");
    return;
  }
  // 4. Hapus to-do yang dipilih dari array `todos`
  todos.splice(deleteNumberTodo - 1, 1);
...
function listTodos() {
  ...
  if (todos.length === 0) {
    ...
    todos.forEach((object, index) => {
      let status = object.isCompleted ? "[DONE]" : "[ACTIVE]";
      console.log(`${index + 1}. ${status} | ${object.text}`);
    });
  }
...
function runTodoApp() {
  ...
  while (running) {
    ...
    let inputMenu = prompt(
      "Input Command : add, complete, delete, list, exit "
    );
    if (!inputMenu) {
      console.log("should handle all command variants");
      return;
    }
    ...
    switch (inputMenu) {
      ...
      case "exit":
      case "5":
        let mockExit = prompt("Do you want to exit : yes/no");
        if (!mockExit) {
          return;
        }
        if (mockExit === "yes") {
